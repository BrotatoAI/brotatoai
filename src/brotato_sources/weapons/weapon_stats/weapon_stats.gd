...
func get_text(base_stats: Resource) -> String:
	...
	text += Text.text("DAMAGE_FORMATTED", [get_col_a() + tr("STAT_DAMAGE") + col_b, get_damage_text(base_stats)])
	text += "\n" + Text.text("CRITICAL_FORMATTED", [get_col_a() + tr("CRITICAL") + col_b, get_crit_damage_text(base_stats), get_crit_chance_text(base_stats)])
	text += "\n" + Text.text("STAT_FORMATTED", [get_col_a() + tr("COOLDOWN") + col_b, get_cooldown_text(base_stats)])
...
func get_range_text(base_stats: Resource) -> String:
	...
	if max_range != base_stats.max_range:
		text += get_init_a() + str(base_stats.max_range) + col_b
...
func get_dmg_text_with_scaling_stats(base_stats: Resource, p_scaling_stats: Array, nb_projectiles: int) -> String:
	...
	var a = get_signed_col_a(damage, base_stats.damage)
