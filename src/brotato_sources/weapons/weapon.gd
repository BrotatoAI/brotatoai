...
func get_next_cooldown()->float:
	if is_big_reload_active():
		return current_stats.cooldown * current_stats.additional_cooldown_multiplier
	var max_rand = get_max_rand_cooldown()
	return rand_range(max(1, current_stats.cooldown - max_rand), current_stats.cooldown + max_rand)


func get_max_rand_cooldown()->float:
	var weapon_count = min(_parent.get_nb_weapons(), 6)
	return min(weapon_count * current_stats.cooldown / 5.0, weapon_count * 5.0)


func is_big_reload_active()->bool:
	return current_stats.additional_cooldown_every_x_shots != - 1 and _nb_shots_taken % current_stats.additional_cooldown_every_x_shots == 0
