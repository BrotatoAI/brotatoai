...
func manage_special_spawn_projectile(
	...
	instance_node:Node = null
)->Node:
	var pos = entity_from.global_position

	if p_weapon_stats.shooting_sounds.size() > 0:
		SoundManager2D.play(Utils.get_rand_element(p_weapon_stats.shooting_sounds), pos, 0, 0.2)
