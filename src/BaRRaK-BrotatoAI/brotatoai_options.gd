...
var enable_fps_meter
var enable_server_status
...
func _input(event):
	if event is InputEventKey:
		if event.shift and event.scancode == KEY_SPACE and option_cooldown < 0.0:
			...
			emit_signal("setting_changed", "ENABLE_AUTOBATTLER", enable_autobattler, "BaRRaK-BrotatoAI")
			...
			if $"/root/ModLoader".has_node("dami-ModOptions"):
				...
				if mod_configs.has("BaRRaK-BrotatoAI"):
					var config = mod_configs["BaRRaK-BrotatoAI"]
					config["ENABLE_AUTOBATTLER"] = enable_autobattler
					mod_configs_interface.on_setting_changed("ENABLE_AUTOBATTLER", enable_autobattler, "BaRRaK-BrotatoAI")
...
func load_mod_options():
	...
	if not $"/root/ModLoader".has_node("dami-ModOptions"):
		print_debug('no dami-ModOptions found')
		return
	...
	if mod_configs.has("BaRRaK-BrotatoAI"):
		...
		if config.has("FPS_METER"):
			enable_fps_meter = config["FPS_METER"]
			
		if config.has("SERVER_STATUS"):
			enable_server_status = config["SERVER_STATUS"]
